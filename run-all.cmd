...
REM   API  -> http://localhost:8000   (FastAPI, from api\)
REM   Web  -> http://localhost:3000   (Next.js, from web\)
...
start "Remax API  (port 8000)" cmd /k "cd /d "%~dp0api" && "%~dp0config\Scripts\python.exe" -m uvicorn app.main:app --reload --port 8000"

REM Give the API a moment to bind before the web app starts calling it.
timeout /t 3 /nobreak >nul

start "Remax Web  (port 3000)" cmd /k "cd /d "%~dp0web" && npm run dev"
